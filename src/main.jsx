  ...
  return <>
    ...
    <div className="coin-grid">
      <section className="panel"><div className="panel-heading"><h2>How $ROUTE is bought</h2><Path size={22} /></div>
        ...
        <div className="coin-route-status"><span>{!buyback?.enabled ? 'Buybacks are paused.' : buyback.blockedReason ? 'Buybacks are awaiting setup.' : 'Buybacks are running.'}</span><span className={`status-pill ${buyback?.enabled ? 'bonded' : 'pending'}`}>{!buyback?.enabled ? 'Paused' : buyback.blockedReason ? 'Setup pending' : 'Running'}</span></div>
      </section>
...
function Admin() {
  ...
  return <>
    ...
    {status && <div className="admin-grid">
      <section className="panel admin-panel"><div className="panel-heading"><h2>Buyback</h2><span className={`status-pill ${buyback?.enabled ? 'bonded' : 'pending'}`}>{buyback?.enabled ? (buyback.blockedReason ? 'Blocked' : 'Running') : 'Stopped'}</span></div>
