      ...
      <aside className="launch-preview"><div className="preview-sticky"><PayoutPreview recipients={draft.recipients} />
        <div className="token-preview"><div className={`token-art ${image ? 'with-art' : ''}`}>{image ? <img src={image.url} alt={`${draft.name || 'Your token'} artwork preview`} /> : <ImageSquare size={65} weight="light" />}</div>
          <div className="token-preview-body"><div className="token-title"><h2>{draft.name || 'Your token name'}</h2><span>${draft.ticker || 'TICKER'}</span></div><PumpBadge />
            ...
            <div className="preview-recipients-title"><span>Your fee route</span><UsersThree size={20} /></div><Distribution recipients={draft.recipients} small />
...
function ReviewDialog({ open, onClose, draft, image }) {
  ...
  }} onClick={event => { if (event.target === ref.current) { const rect = ref.current.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) onClose(); } }}><div className="review-header"><span className="icon-tile"><Path size={23} /></span><button className="icon-button" onClick={onClose} aria-label="Close launch review"><X size={20} /></button></div><h2 id="review-title">Your route, ready to review.</h2><p id="review-description">Check the details of your token and its recipients.</p><div className="review-token">{image && <img src={image.url} width="56" height="56" alt="Token artwork" />}<div><strong>{payload.name}</strong><span>${payload.symbol}</span></div><PumpBadge /></div><Distribution recipients={draft.recipients} /><dl className="review-details"><div><dt>Dev buy</dt><dd>{draft.devBuy} SOL</dd></div><div><dt>Website</dt><dd>{payload.website}</dd></div><div><dt>Launch fee</dt><dd>Not quoted yet</dd></div></dl><div className="review-notice"><LockSimple size={18} /><p><strong>Live launches are not enabled yet.</strong>Your draft is ready to review. No token has been created and no payment will be taken.</p></div><button className="button primary" disabled><Wallet size={17} />Payments coming soon</button><button className="text-button back-to-edit" onClick={onClose}>Back to editing</button></dialog>;
}
