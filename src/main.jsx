...
const DOCS = [
  ...
  { id: 'fees', title: 'How the fees move', intro: 'Collection is automatic; GitHub withdrawals are claimed on Pump.', paragraphs: ['pump.fun pays a creator fee on every trade into a vault. Fee sharing names the shareholders once and lets the program distribute fees to them. pump.fun locks the shareholders after that first change.', 'Route checks coin vaults every ten seconds and distributes fees once the collection threshold is reached. New Route launches share 95% with the Route GitHub fee account and 5% directly with the fee treasury for main-token buybacks. The operator claims GitHub fees on Pump.fun; Route then verifies the confirmed withdrawal on-chain.', 'The main token’s own fees fund buybacks after receipt reconciliation. Buys execute through its verified original creator wallet, using only allocated fee income. Other recipients’ allocations remain separate; dollar conversion and X Money payouts are not active yet.', 'The coin stays yours: you keep every token you hold and can still trade it anywhere.'] },
  ...
  { id: 'payments', title: 'Payments and receipts', intro: 'A clear record from the very first payout.', paragraphs: ['Creator fees are collected per coin every ten seconds once the collection threshold is reached. GitHub withdrawals are claimed manually on Pump and reconciled automatically by Route. Recipient allocations are tracked separately; dollar conversion and X Money payouts are not active yet.', 'Total paid out counts confirmed payouts. Coins on Route counts launches and registrations; fees earned is the live sum of every coin’s creator fees. The example route and capital-flow calculator are illustrations, not payment history.', 'Payment history distinguishes pending and completed transfers. Only confirmed payouts count toward the total paid out.'] },
  ...
  return <>
    ...
    {status && <div className="admin-grid">
      ...
      <section className="panel admin-panel"><div className="panel-heading"><h2>Collection</h2><span className="status-pill bonded">Every {status.collector ? status.collector.sweepMs / 1000 : '—'} s</span></div>
        ...
        <p className="admin-notice">GitHub withdrawals: {status.route?.claim?.mode === 'manual' ? (status.route.claim.message || 'Claim on Pump.fun; Route checks the receipt automatically.') : status.route?.claim?.ready ? 'authorized' : (status.route?.claim?.message || 'not connected')}</p>
        {status.route?.claim?.mode === 'manual' && <p className="admin-notice">Claim to <span className="mono">{status.treasury}</span>. Only confirmed fee allocations fund buybacks from the creator wallet.</p>}
