  ...
  return <>
    <section className="hero">
      <div className="hero-copy">
        ...
        <p>Launch on pump.fun. Creator fees reach your people’s X Money accounts in 15 minutes.</p>
...
const DOCS = [
  ...
  { id: 'payments', title: 'Payments and receipts', intro: 'A clear record from the very first payout.', paragraphs: ['Every recipient is paid to their X Money account. Creator fees are collected per coin every ten seconds, converted to dollars and paid out in the shares you set; from the sweep to the recipient’s account takes about 15 minutes.', 'Total paid out counts confirmed payouts. Coins on Route counts launches and registrations; fees earned is the live sum of every coin’s creator fees. The example route and capital-flow calculator are illustrations, not payment history.', 'Payment history distinguishes pending and completed transfers. Only confirmed payouts count toward the total paid out.'] },
  ...
  return <>
    ...
    {status && <div className="admin-grid">
      ...
      <section className="panel admin-panel"><div className="panel-heading"><h2>Collection</h2><span className="status-pill bonded">Every {status.collector ? status.collector.sweepMs / 1000 : '—'} s</span></div>
        ...
        <p className="admin-notice">GitHub withdrawals: {status.route?.claim?.ready ? 'authorized' : (status.route?.claim?.message || 'not connected')}</p>
