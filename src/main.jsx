...
const Money = ({ sol, price, usd = toUsd(sol, price) }) => (usd != null ? <>{fmtUsd(usd)}</> : sol == null || !Number.isFinite(sol) ? <>—</> : <span className="sol">{fmtSol(sol)}</span>);
...
function Metrics({ large = false }) {
  ...
  return <dl className={`metrics ${large ? 'metrics-large' : ''}`}>
    <div><dt>Coins launched</dt><dd>{live.loaded ? coins.length : stats.coins}</dd></div>
...
function Distribution({ recipients = EXAMPLE_BASKET, small = false, resolve = () => null, earned = null }) {
  ...
  return <div className={`distribution ${small ? 'small' : ''}`}>
    ...
    <div className="distribution-list">{recipients.map((r, i) => <div className="distribution-person" key={r.id}><Avatar index={i} handle={r.handle} small={small} profile={resolve(r.handle)?.profile || r} /><span>{normalizeHandle(r.handle) ? `@${normalizeHandle(r.handle)}` : `Recipient ${i + 1}`}</span>{earned && <em className="distribution-earned">{earned(r)}</em>}<strong>{r.share || '0'}<span>%</span></strong></div>)}</div>
  </div>;
}
...
function RegisterCoin({ register, setRegister, openChooser }) {
  ...
  const already = Boolean(coin?.record?.recipients?.length) && ['active', 'detected'].includes(coin?.record?.route?.status);
  const onChain = coin?.onRoute && !already;
    ...
    <div className="register-layout">
      <form className="register-form" onSubmit={flow.stage === 'done' ? event => event.preventDefault() : coin && !already && !blocked ? submit : lookup} noValidate>
        <section className="form-section">
          ...
          {coin && onChain && <div className="register-note good"><CheckCircle size={18} /><span>This coin already shares its fees with Route. Choose who receives them; no transaction is needed.</span></div>}
        ...
        {coin && !already && !blocked && <div className="form-submit">
          ...
          <p>{onChain ? 'Saved instantly; the fee sharing is already on-chain.' : 'One transaction, signed by the creator wallet. pump.fun locks the route after this.'}</p>
        </div>}
...
function CoinPage({ mint, openChooser }) {
  ...
  const routeActive = ['active', 'detected'].includes(coin.route?.status);
  const detected = coin.route?.status === 'detected';
  ...
  return <>
    ...
    <div className="coin-grid">
      <section className="panel"><div className="panel-heading"><h2>Fee route</h2><UsersThree size={22} /></div><div style={{ marginTop: 22 }}>{coin.recipients?.length ? <Distribution recipients={recordRecipients(coin)} resolve={() => null} earned={recipient => { const poolSol = (state?.feesSol || 0) * ((coin.shares?.othersBps ?? 10000) / 10000); const sol = poolSol * (toBasisPoints(recipient.share) || 0) / 10000; return <Money sol={sol} price={live.sol} />; }} /> : <div className="quiet-empty" style={{ padding: '8px 0 0' }}><UsersThree size={23} /><div><h3>Recipients not set yet</h3><p>The creator sets who receives this coin’s fees from the Existing coin tab.</p></div></div>}</div>
        {coin.recipients?.length > 0 && <p className="distribution-note">Earned so far, per person, from this coin’s fees and their share.</p>}
        <div className="coin-route-status"><span>{detected ? 'On-chain fee sharing is locked to Route. The creator picks the recipients.' : routeActive ? 'On-chain fee sharing is active and locked to Route.' : coin.route?.status === 'failed' ? 'The fee route transaction did not land.' : coin.route?.status === 'unknown' ? 'The fee route is still unconfirmed.' : 'Fee route pending.'}</span><span className={`status-pill ${routeActive ? 'bonded' : 'pending'}`}>{routeActive ? 'On Route' : 'Pending'}</span></div>
        {detected && account && account.address === coin.wallet && <div style={{ marginTop: 18 }}><Link className="button primary" to="/register" onClick={() => { try { localStorage.setItem(REGISTER_KEY, JSON.stringify({ mint: coin.mint, recipients: freshRecipients() })); } catch { /* optional */ } }}><UsersThree size={17} />Choose the recipients</Link></div>}
...
function MainCoin() {
  ...
  return <>
    ...
    <div className="coin-tiles">
      ...
      <div className="coin-tile"><span>$ROUTE bought</span><strong>{tokens ? tokens.toLocaleString('en-US', { maximumFractionDigits: 0 }) : '0'}</strong><small>{buyback?.lastAt ? `last buy ${timeAgo(buyback.lastAt)}` : 'held by the dev wallet'}</small></div>
    ...
    <div className="coin-grid">
      <section className="panel"><div className="panel-heading"><h2>How $ROUTE is bought</h2><Path size={22} /></div>
        <p className="doc-intro" style={{ marginTop: 18 }}>Every ten seconds Route sweeps every coin. 5% of every coin’s fees and 100% of $ROUTE’s own fees buy $ROUTE on pump.fun from the dev wallet.</p>
        <div className="coin-route-status"><span>{buyback?.armed ? 'Buybacks will start once launch and fee sharing are verified.' : !buyback?.enabled ? 'Buybacks are paused.' : buyback.blockedReason ? 'Buybacks are awaiting setup.' : 'Buybacks are running.'}</span><span className={`status-pill ${buyback?.enabled ? 'bonded' : 'pending'}`}>{buyback?.armed ? 'Waiting' : !buyback?.enabled ? 'Paused' : buyback.blockedReason ? 'Setup pending' : 'Running'}</span></div>
      </section>
...
const DOCS = [
  { id: 'overview', title: 'A little coin. A bigger circle.', intro: 'Route is a way to launch a pump.fun token with creator fees shared between the people you choose.', paragraphs: ['Instead of choosing one recipient, route the fees to up to five X accounts. Set a percentage for each person and review the complete allocation before launch.', 'Connect a Solana wallet, build your route and create the coin on pump.fun. Its creator fees flow to Route through pump.fun’s fee sharing. Collection and buybacks are supported; X Money payouts are not active yet.'] },
  { id: 'launching', title: 'Create your token', intro: 'Keep the idea simple. Make the details your own.', paragraphs: ['Add your token’s name, ticker and image. A description and an X profile or post link are optional. The website field automatically links to your token’s page on Route.', 'Set an optional dev buy in SOL, or leave it at zero. The launch flow is designed for a single creator wallet. Bundle buys are not offered.', 'Review launch checks your draft, verifies every recipient on X and opens a summary. Launch on pump.fun asks your wallet to confirm two transactions at once: one creates the coin, the next puts its fees on Route. Your wallet pays the dev buy and the Solana network fees; Route charges no launch fee today.'] },
  { id: 'fees', title: 'How the fees move', intro: 'Creator fees are collected automatically to the Route fee wallet.', paragraphs: ['pump.fun pays a creator fee on every trade into a vault. Fee sharing names the shareholders once and lets the program distribute fees to them. pump.fun locks the shareholders after that first change.', 'Route checks coin vaults every ten seconds and distributes fees once the collection threshold is reached. In direct-wallet mode, new Route launches send fees to the Route fee wallet. A 5% share of other coins’ fees funds main-token buybacks; the remaining recipient pool is accounted for separately.', 'The main token’s own fees fund buybacks after receipt reconciliation. Buys execute through its verified original creator wallet, using only allocated fee income. Other recipients’ allocations remain separate; dollar conversion and X Money payouts are not active yet.', 'The coin stays yours: you keep every token you hold and can still trade it anywhere.'] },
  ...
  { id: 'payments', title: 'Payments and receipts', intro: 'A clear record from the very first payout.', paragraphs: ['Creator fees are collected per coin every ten seconds once the collection threshold is reached. In direct-wallet mode, Route collects and verifies fees on-chain without a manual claim. Recipient allocations are tracked separately; dollar conversion and X Money payouts are not active yet.', 'Total paid out counts confirmed payouts. Coins on Route counts launches and registrations; fees earned is the live sum of every coin’s creator fees. The example route and capital-flow calculator are illustrations, not payment history.', 'Payment history distinguishes pending and completed transfers. Only confirmed payouts count toward the total paid out.'] },
  ...
  return <>
    ...
    {status && <div className="admin-grid">
      <section className="panel admin-panel"><div className="panel-heading"><h2>Buyback</h2><span className={`status-pill ${buyback?.enabled ? 'bonded' : 'pending'}`}>{buyback?.armed ? 'Waiting for launch' : buyback?.enabled ? (buyback.blockedReason ? 'Blocked' : 'Running') : 'Stopped'}</span></div>
        <dl className="review-details"><div><dt>Main coin</dt><dd className="mono">{buyback?.mainCoin ? shortAddress(buyback.mainCoin) : 'not set'}</dd></div><div><dt>Buyback wallet</dt><dd className="mono">{buyback?.wallet ? `${shortAddress(buyback.wallet)}${buyback.separate ? '' : ' (treasury)'}` : 'not set'}</dd></div><div><dt>Confirmed buyback allocation</dt><dd className="sol">{sol(buyback?.owedLamports)}</dd></div>{buyback?.separate && <div><dt>Forwarded so far</dt><dd className="sol">{sol(buyback?.forwardedLamports)}</dd></div>}{buyback?.separate && <div><dt>Waiting to forward</dt><dd className="sol">{sol(buyback?.toForwardLamports)}</dd></div>}<div><dt>Buyback wallet balance</dt><dd className="sol">{sol(buyback?.walletLamports)}</dd></div><div><dt>Available now</dt><dd className="sol">{sol(buyback?.availableLamports)}</dd></div><div><dt>Spent on buybacks</dt><dd className="sol">{sol(buyback?.spentLamports)}</dd></div><div><dt>Minimum per buy</dt><dd className="sol">{sol(buyback?.minLamports)}</dd></div><div><dt>Backup</dt><dd>{buyback?.backup === 'pumpportal' ? 'PumpPortal' : 'None'}</dd></div><div><dt>Last run</dt><dd>{buyback?.lastRun ? timeAgo(buyback.lastRun) : 'never'}</dd></div></dl>
        {buyback?.blockedReason && <p className="admin-notice">{buyback.blockedReason}</p>}
        {buyback?.creator?.verified && <p className="admin-notice">Buyback wallet verified against the token?s creation transaction.</p>}
        ...
        <div className="admin-actions">
          {buyback?.enabled || buyback?.armed ? <button type="button" className="button secondary" disabled={!!busy} onClick={() => act('stop', () => adminBuyback(token, { enabled: false }))}><Stop size={17} weight="fill" />Stop buybacks</button>
            : <button type="button" className="button primary" disabled={!!busy || !buyback?.mainCoin || !buyback?.creator?.verified || !!buyback?.blockedReason} onClick={() => act('start', () => adminBuyback(token, { enabled: true }))}><Play size={17} weight="fill" />Start buybacks</button>}
          {!buyback?.enabled && !buyback?.armed && <button type="button" className="button secondary" disabled={!!busy || !buyback?.configured} onClick={() => act('arm', () => adminBuyback(token, { armed: true }))}>Start after launch</button>}
          <button type="button" className="button secondary" disabled={!!busy || !buyback?.mainCoin || !buyback?.creator?.verified || !!buyback?.blockedReason} onClick={() => act('run', () => adminBuybackRun(token))}>Buy now</button>
          <span className="admin-empty">Buy route: Pump / PumpSwap SDK.</span>
        </div>
      ...
      <section className="panel admin-panel"><div className="panel-heading"><h2>Collection</h2><span className="status-pill bonded">Every {status.collector ? status.collector.sweepMs / 1000 : '—'} s</span></div>
        <dl className="review-details"><div><dt>Treasury</dt><dd className="mono">{status.treasury ? shortAddress(status.treasury) : '—'}</dd></div><div><dt>Coins on Route</dt><dd>{status.coins?.coins ?? 0}</dd></div><div><dt>Collected to date</dt><dd className="sol">{sol(status.coins?.collectedLamports)}</dd></div>{status.route?.github && <><div><dt>GitHub</dt><dd>{status.route?.github ? `${status.route.github.login} ${status.route.github.ready ? '' : '(account missing)'}` : 'not set'}</dd></div><div><dt>Waiting in GitHub account</dt><dd className="sol">{fmtSol(status.route?.unclaimedSol)}</dd></div><div><dt>Claimed on pump.fun</dt><dd className="sol">{fmtSol(status.route?.claimedSol)}</dd></div></>}</dl>
        {status.route?.github && <p className="admin-notice">GitHub withdrawals: {status.route?.claim?.mode === 'manual' ? (status.route.claim.message || 'Claim on Pump.fun; Route checks the receipt automatically.') : status.route?.claim?.ready ? 'authorized' : (status.route?.claim?.message || 'not connected')}</p>}
        {status.route?.github && status.route?.claim?.mode === 'manual' && <p className="admin-notice">Claim to <span className="mono">{status.treasury}</span>. Only confirmed fee allocations fund buybacks from the creator wallet.</p>}
        {!status.route?.github && <p className="admin-notice">Automatic fee collection to <span className="mono">{status.treasury}</span>. No GitHub sign-in or manual claim is needed. Buybacks use verified fee income in the creator wallet.</p>}
        {status.discovery?.main && <p className="admin-notice">Main coin registration: {status.discovery.main.status === 'registered' ? 'registered automatically; fee collection is tracking this coin' : status.discovery.main.message}</p>}
...
function App() {
  ...
  const label = coinMint ? (liveState.coins[coinMint]?.name || 'Coin') : path === '/register' ? 'Register a coin' : path === '/admin' ? 'Admin' : path === '/route' ? '$ROUTE' : path === '/payments' ? 'Coins' : ROUTES.find(route => route[0] === path)?.[1] || 'Page not found';
  useEffect(() => {
