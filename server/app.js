...
export function createApp({ store, service, xLookup, engine, dataDir, distDir, origin, treasury, price = null, watcher = null, collector = null, buyback = null, socialClaimer = null, detector = null, adminToken = '', github = null, setup = null, log = console }) {
  ...
  app.get('/api/health', (req, res) => res.json({ ok: true, origin, treasury: treasury?.toBase58() || null, devBuys: engine.devBuysEnabled, collector: collector?.enabled ? collector.address : null, buybackWallet: buyback?.wallet || null, sweepMs: collector?.enabled ? collector.sweepMs : null, watching: watcher?.size() ?? 0, sol: price?.get() || null, route: routeInfo(), dataDir, ...store.stats() }));
