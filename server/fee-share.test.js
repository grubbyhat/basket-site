...
test('the fee-route transaction is one creator-signed transaction that fits without a lookup table', async () => {
  ...
  const { transaction, bytes } = await compileRoute({ mint, creator, shareholder: treasury, graduated: false, blockhash: BLOCKHASH });
  ...
  const instructions = await routeInstructions({ mint, creator, shareholder: treasury, graduated: false });
  assert.equal(instructions.length, 2, 'create config + set shares');
  const graduated = await routeInstructions({ mint, creator, shareholder: treasury, graduated: true });
...
test('token metadata is read from the Token-2022 mint and inspect describes the coin', async () => {
  ...
  const coin = await inspectCoin({ connection, shareholder: treasury, mint: mint.toBase58(), fetchImpl: async () => ({ ok: true, json: async () => ({ image: 'ipfs://QmImage' }) }) });
  ...
  await assert.rejects(inspectCoin({ connection, shareholder: treasury, mint: 'nope' }), error => error.status === 400);
  await assert.rejects(inspectCoin({ connection, shareholder: treasury, mint: Keypair.generate().publicKey.toBase58() }), error => error.status === 404);
});
